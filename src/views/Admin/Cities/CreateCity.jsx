...
class CreateCity extends React.Component {
  ...
  handleSubmit() {
    if (this.isValidated()) {
      ...
      createCity(dataCity).then(responseSaveCity => {
        if (responseSaveCity.data.message === "success") {
          this.setState({
            messageError: null,
            successMessage: `Ciudad Creada con Éxito`
          });
          setTimeout(() => {
          ...
          <Card>
            <CardHeader color="warning" text>
              <CardText color="warning">
                <h4 className={classes.cardTitle}>Crear Ciudad</h4>
              </CardText>
            </CardHeader>
            <CardBody>
              <form>
                <GridContainer>
                  <GridItem xs={12} sm={4}>
                    <CustomInput
                      ...
                      inputProps={{
                        onChange: event =>
                          this.change(event, "cityName", "length", 5),
                        type: "text",
                        endAdornment:
...
export default withStyles(validationFormsStyle)(CreateCity);
