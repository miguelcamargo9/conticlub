...
class EditBrand extends React.Component {
  ...
  handleSubmit() {
    if (this.isValidated()) {
      ...
      updateBrand(dataBrand).then(responseSaveBrand => {
        if (responseSaveBrand.data.message === "success") {
          this.setState({
            messageError: null,
            successMessage: `Marca ${this.state.brandName} Editado con Éxito`
          });
          setTimeout(() => {
  ...
  change(event, stateName, type, stateNameEqualTo) {
    ...
    console.log("stateNameEqualTo", stateNameEqualTo);
  ...
  render() {
    ...
    const { messageError, successMessage, brandName } = this.state;
          ...
          <Card>
            <CardHeader color="warning" text>
              <CardText color="warning">
                <h4 className={classes.cardTitle}>Editar Marca</h4>
              </CardText>
            </CardHeader>
            <CardBody>
              <form>
...
export default withStyles(validationFormsStyle)(EditBrand);
